...
file <- read.csv(fname)
# mix of categorical variables and numeric variables that are quasi identifiers
...
sdcInitial <- createSdcObj(dat=file,
                      ...
                      # this is easy bc we alreaady identified them above!
                      # These are to be removed and not counted in the computation of k-anon.
                      # these are the quasi identifiers
                       keyVars = c("zip","age", "sex","race","ethn", "snap", "income", "learn_dis","phys_dis"),
                      # What are the numeric quasi idenfitiers?
                       numVars=c("math_sc", "read_sc"),
                       weightVar=NULL,
                       hhId=NULL,
                       strataVar=NULL,
                       pramVars=NULL,
                       # What do we want to exclude completely?
                       # These are the direct idenfiers
                       # DOB in this data set bc the likelihood of dupilcate bdays is not high for this ds
                       # therefore, we are removing birthday's in this investigation to uphold anonymity
                       # "stu_id", is considered a pseudonym 
                       excludeVars=c("ssn", "name", "dob"),
                       # for reproducibility
                       seed=0,
                       # we don't want to randomize
...
# the 81% says that removing the direct identifier, we will have a very high score of ability to reidentity
sdcInitial@risk$global$risk

# what's the risk for an individual?
head(sdcInitial@risk$individual, 3)
```
...
#Recode Age 
#Top
sdcInitial <- groupAndRename(obj= sdcInitial, var= c("age"), before=c(9), after=c(10))  
#Bottom
...
sdcInitial <- kAnon(sdcInitial, k= 5)
...
#K-anon
# We lost a lot of data using kAnon. 
# this will make it difficult for users in the future to use the data
print(sdcInitial, 'kAnon')
print(sdcInitial, 'ls')
...
print(sdcInitial, 'ls')
```
A ton of NA values were introduced because our data is more anonymize the info. This level 5 really removes a ton of info from this 100 observation data set. 
```{r}
